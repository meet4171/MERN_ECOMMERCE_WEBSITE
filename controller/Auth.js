    ...
    try {
        ...
        req.login(token, function (err) {
            if (err) {
                ...
                res.cookie('jwt', token, { expires: new Date(Date.now() + 3600000), httpOnly: true }).status(201).json(sanitizeUser(response))
            }
        });
    } catch (error) {
...
exports.loginUser = async (req, res) => {
    ...
    res.cookie('jwt', token, { expires: new Date(Date.now() + 3600000), httpOnly: true, path: '/', }).status(200).json(req.user);
};
